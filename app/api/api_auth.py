...
@auth_router.post(
    ...
    status_code=201,
    responses={
        status.HTTP_409_CONFLICT: {"description": "Имя пользователя занято"},
...
def register(
    ...
    """
    existing_user = users_service.get_by_username(
        cursor=cursor, username=new_user_payload.username
    )
    if existing_user:
        raise ConflictHTTPException(detail="Имя пользователя занято")
