...
def to_dream_schema(dream) -> schema.Dream:
    return schema.Dream(
        id=dream.id,
...
@dreams_router.get(
    ...
    responses={
        status.HTTP_422_UNPROCESSABLE_CONTENT: {
            "description": "Параметры запроса не валидные"
        },
    },
)
def get_dreams_list(
    ...
    return schema.MultipleDreams(
        dreams=[to_dream_schema(dream) for dream in dreams],
...
@dreams_router.post(
    ...
    responses={
        status.HTTP_401_UNAUTHORIZED: {
            "description": "Ошибка токена или пользовательских данных"
        },
        status.HTTP_409_CONFLICT: {
            "description": "Пользователь уже добавлял этот сон"
        },
        status.HTTP_422_UNPROCESSABLE_CONTENT: {
...
@dreams_router.get(
    ...
    responses={
        ...
        status.HTTP_422_UNPROCESSABLE_CONTENT: {
            "description": "Идентификатор не валиден"
        },
    },
)
def get_dream(
...
@dreams_router.delete(
    ...
    responses={
        ...
        status.HTTP_403_FORBIDDEN: {
            "description": "Пользователь не является автором сна"
        },
        status.HTTP_422_UNPROCESSABLE_CONTENT: {
            "description": "Идентификатор не валиден"
        },
