...
def set_role(*, session: Session, username: schema.UsernameType, role: schema.RoleType) -> None:
	"""
	:session: сессия подключения к базе данных
	:username: уникальный юзернейм пользователя
...
def update(
	*, session: Session, username: schema.UsernameType, update_data: schema.UserUpdate
