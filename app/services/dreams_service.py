...
def get_by_id(*, session: Session, id: schema.IndexType) -> schema.Dream | None:
    dream = session.execute(
        text('SELECT id, author, description, created_at FROM dreams WHERE id = :id'),
        {'id': id}
    ).fetchone()
    ...
    favorited_by = session.execute(
        text('SELECT username FROM dream_favorite WHERE dream_id = :id'),
        {'id': id}
    ).scalars().all()
...
def get_list(
    *,
    ...
    count = ('SELECT COUNT(id) FROM dreams'
                   + (' JOIN dream_favorite ON dream_favorite.dream_id = dreams.id' if favorited else '')
                   + ' WHERE TRUE'
                   + (' AND author LIKE :author' if author else '')
                   + (' AND description LIKE :search' if search else '')
    ...
    query_params = {k: v for k, v in {'author': f'%{author}%', 'search': f'%{search}%' if search else None,
                                'favorited': f'%{favorited}%' if favorited else None,
                                'limit': limit, 'offset': offset}.items() if v is not None}
    count_params = {k: v for k, v in {'author': f'%{author}%', 'search': f'%{search}%' if search else None,
...
def create(*, session: Session, new_dream: schema.NewDream, author: schema.UserProfile) -> schema.Dream:
    try:
        dream = session.execute(
            text("INSERT INTO dreams (author, description, created_at) VALUES (:username, :description, datetime('now')) RETURNING id, created_at"),
...
def delete(*, session: Session, dream_id: schema.IndexType) -> None:
    session.execute(text('DELETE FROM dream_favorite WHERE dream_id = :dream_id'), {'dream_id': dream_id})
...
def delete_by_username(*, session: Session, username: schema.UsernameType) -> None:
    session.execute(text('DELETE FROM dream_favorite WHERE username = :username'), {'username': username})
...
def favorite(*, session: Session, dream: schema.Dream, user: schema.UserProfile) -> None:
    session.execute(
        text('INSERT INTO dream_favorite (username, dream_id) VALUES (:username, :dream_id) ON CONFLICT (username, dream_id) DO NOTHING;'),
